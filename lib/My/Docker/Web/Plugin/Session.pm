package My::Docker::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => 'vXRXrOpIFjiJlyRi282gmyDdrQ289Xna',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => 'HkrOgsUizVUXNHEgQglwFLw-gGy1LILs',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for My::Docker.
